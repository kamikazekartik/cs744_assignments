...
files = [
	...
	'output/v100_fullmodel_2/results_resnet50_Cifar10_batchsize128_precision8bit.csv'
]

data = []
data.append(['model', 'dataset', 'precision', 'epoch', 'training_loss', 'test_acc', 'epoch_train_time', 'total_train_time'])
for file in files:
  tmp = file.replace('output/v100_fullmodel_2/results_' ,'').replace('output/v100_convergence/results_' ,'').replace('precision','').replace('.csv','')
  tmp = tmp.split('_')
  model = tmp[0]
  dataset = tmp[1]
