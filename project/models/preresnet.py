...
class resnet(nn.Module):
    def __init__(self, depth=101, dataset='cifar10', cfg=None):
...
def preresnet(pruned_model_path, reset_weights, **kwargs):
    r'''
    ...
    '''
    if pruned_model_path == None:
        print('Building unpruned resnet from scratch')
        model = resnet()
        return model
    print('Loading pruned model from:', pruned_model_path)
    checkpointed_model = torch.load(pruned_model_path)
    kwargs = {}
    kwargs['cfg'] = checkpointed_model['cfg']
